...
bool SvgaPlayerPrivate::loadSvgaFile()
{
	...
	if (m_canvas)
	{
		...
		if (video)
		{
			...
			this->m_width = video->width();
			this->m_height = video->height();
		}
	}
		...
		for (int i = 0; i < sprites.size(); i++)
		{
			...
			if (item)
			{
				...
				drawItem.clipPath = item->clipPath();
                                ...
                                const QPixmap& pix = m_resource.getDynamicImage(sprites[i]->imageKey());
				if (pix.isNull())
				{
...
int SvgaPlayerPrivate::videoWidth()
{
	return this->m_width;
}

int SvgaPlayerPrivate::videoHeight()
{
...
void SvgaPlayer::getSvgaVideo(int& width,int& height)
{
	if(m_ptr)
	{
		width = m_ptr->videoWidth();
		height = m_ptr->videoHeight();
	}
